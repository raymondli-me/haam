...
"""
Aligned Word Cloud Generation with Proper Y/HU/AI Labels (Version 2)
===================================================================
This version fixes the get_topic_means error by calculating quartiles directly.
Labels now properly show topic quartile positions that match the validity coloring.
"""
...
print("="*80)
print("ALIGNED WORD CLOUD GENERATION WITH PROPER Y/HU/AI LABELS (V2)")
print("="*80)
...
def get_topic_quartile_positions_v2(haam_instance, topic_ids):
    """
    Calculate quartile positions for topics by averaging Y/HU/AI values.
    Returns H/M/L for each measure based on where topics fall in the distribution.
    """
    if not topic_ids:
        return {'Y': '?', 'HU': '?', 'AI': '?'}
...
def get_pc_associations(haam_instance, pc_idx):
    """Get Y/HU/AI associations for a PC (for reference only)."""
    associations = {}
    try:
...
def create_aligned_word_cloud_with_info(haam_instance, pc_idx, k=3, max_words=150, 
    ...
    high_quartiles = get_topic_quartile_positions_v2(haam, high_topic_ids)
    ...
    low_quartiles = get_topic_quartile_positions_v2(haam, low_topic_ids)
    ...
    plt.suptitle(f'PC{pc_idx + 1} Word Clouds with Aligned Validity Labels', 
...
output_dir = 'haam_wordclouds_aligned_v2'
os.makedirs(output_dir, exist_ok=True)
...
plt.suptitle('All 15 Principal Components - Aligned Validity Labels', 
...
# Save table
table_path = os.path.join(output_dir, 'pc_table_aligned_v2.png')
...
print("  - Labels now show actual topic quartile positions (H/M/L)")
print("  - Each pole shows its own quartile positions")
print("  - Labels properly align with validity coloring")
print("  - No dependency on missing get_topic_means method")
print("  - Clear distinction between PC coefficients and topic quartiles")

print("\n🔍 How to read the labels:")
print("  - H = Topics in top quartile (≥75th percentile)")
print("  - L = Topics in bottom quartile (≤25th percentile)")
print("  - M = Topics in middle quartiles")
print("  - Labels match the colors: L L L + dark blue = consensus low")